...
const App = ()=> {
  ...
  const handleNew = () =>
  {
    alert("NEW: not implemented zet");
    const maxValue = Math.max(...data.map(item => item.id));
    const newrow = createData(
      maxValue + 1, 'new row item', 111, 1.1, 1, 1.1, "new button", "it!!!", true);
  ...
  {
    //alert("MODAL DIALOG: not implemented yet.");
    //ModalDialog();
  }
  ...
  function setButtonsAll()
  {
    let mainChanged = false;
    for (let i = 0; i < data.length; i++)
    {
      const boolCg = data[i]["rowchanged"];
      if (boolCg === true) mainChanged = true;
    }
    setChanged(mainChanged);
  }

  const saveRow = (rowIndex) => {
    if (data[rowIndex]["rowchanged"] = true)
    {
      ...
      {
        if (newData[rowIndex]["rowchanged"] === true) mainChanged = true;
      }
      setChanged(mainChanged);
      */
  ...
  const cancelRow = (rowIndex) => {
    ...
    if (data[rowIndex]["rowchanged"] = true)
    {
      ...
      // now check the buttons save all, cancel all
      setButtonsAll();

      alert("row was cancelled");
    }
    else
    {
      alert("row was not cancelled");
    }
  };
